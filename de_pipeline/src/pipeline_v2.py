...
def audit_apply(location_groups):
    ...
    @task
    def merge_audit(location_group, **context):
        logging.info("merging audit")
        # return "gs://extracted-bucket-dollar-tree/Raghav/test_files/business_feature_store_all_loc_V1/LOCATION_GROUP="
        return location_group

    audit_location_grp = merge_audit(delete_job(submit_job(location_groups)))
    # denorm_location_group = delete_job(submit_job(location_groups))
...
def start_bfs(location_groups):
    logging.info("waiting for e2e validation completion")
    logging.info(location_groups)
    return location_groups
...
@task
def end_pipeline(location_groups, **context):
    logging.info("waiting for inference completion")
    logging.info(location_groups)
    return location_groups
