...
def preprocessing(spark, input_files, output_path, master_output_path, table_to_process):
    """
    processing the output of validator data , create location groups in it and save to output folder
    """
    print(f"folder list to process {table_to_process}")
    if table_to_process in config.master_tables:
        spark.read.options(header='True', inferSchema='True') \
            .csv(input_files).write.option("header", True).option("compression", "gzip").mode("overwrite") \
...
def main(args=None):
    ...
    spark = get_spark_session()
    input_files = args.input_path.split(",")
    preprocessing(spark, input_files, args.output_path, args.master_output_path, args.table_names)
